...
def dump(number,title,string):
    ...
    with open("result/post/response/TEST " + str(number) + "" + title + ".json", "w")as file:
        file.write(string)
...
def xlsx(number,title,url,send,string,status):
    ...
    wbo.save("result/post/report/test " + str(number)+".xlsx")
...
def start():
    ...
    test_list = os.listdir("testrun/post")
    number = 1
    for test_file in test_list:
        test = os.path.abspath("testrun/post/" + test_file)
