...
def dump(number,test,string):
    ...
    with open("result/get/response/TEST " + str(number) + " " + test + ".json", "w")as file:
        file.write(string)
...
def xlsx(number,test,url,send,string,status,result):
    ...
    wbo.save("result/get/report/test " + str(number) + " " + test + ".xlsx")
...
def run(test,url,send,check,number):
    ...
    try:
        ...
        print("\nRESPONSE BODY: \n")
        print(string)
        print("\nSTATUS CODE: ",status," RESULT: ",result )
...
def start():
    ...
    path = os.listdir("testrun/get")
    number = 1
    for test in path:
        load = os.path.abspath("testrun/get/" + test)
